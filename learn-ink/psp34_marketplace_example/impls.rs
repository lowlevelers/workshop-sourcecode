...
use super::types::NftData;

pub trait MarketInternal: Storage<NftData> + psp34::Internal {
		...
		if transferred_value != self.data::<NftData>().price_per_mint {
			return Err(PSP34Error::Custom(
				NftError::BadMintValue.as_str()
					+ "Required:" + &self.data::<NftData>().price_per_mint.to_string()
					+ ", Supplied:" + &transferred_value.to_string(),
			));
		}
	...
	+ MarketInternal
{
	...
	fn token_uri(&self, id: u64) -> Result<String, PSP34Error> {
		...
		let fid = self.data::<NftData>().fid_list.get(&id).ok_or(PSP34Error::TokenNotExists)?;
	...
	fn balance(&mut self) -> Balance {
		...
		let current_balance =
			balance.checked_sub(Self::env().minimum_balance()).unwrap_or_default();
		current_balance
	}
	...
	fn list(&mut self, id: u64, price: Balance) -> Result<(), PSP34Error> {
		...
		self.data::<NftData>().sale_list.insert(&id, &(price * 1_000_000_000_000));
		Ok(())
	}
		...
		if price != transferred {
			return Err(PSP34Error::Custom(
				NftError::PriceNotMatch.as_str()
					+ "Required:" + &price.to_string()
					+ ", Supplied:" + &transferred.to_string(),
			));
		}

		// Transfer native tokes
		if Self::env().transfer(owner, price).is_err() {
			return Err(PSP34Error::Custom(NftError::TransferNativeTokenFailed.as_str()));
		}
